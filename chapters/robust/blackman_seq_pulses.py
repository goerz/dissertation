...
def create_figure(outfile, pulse_blue_left, pulse_red_left, pulse_blue_right,
    ...
    ax = fig.add_axes(pos)
    tgrid = pulse_blue_right.tgrid # use for all panels
    ax.plot(tgrid, np.abs(pulse_blue_right.amplitude), color=lightblue)
    line, = ax.plot(tgrid, np.abs(pulse_red_right.amplitude), color=red)
    line.set_dashes([8,8])
    set_axis(ax, 'x', 0, 800, 100, minor=4, label='time (ns)')
    set_axis(ax, 'y', 0, 60,  20, minor=2, label='amplitude (MHz)')
    ...
    ax.yaxis.set_label_coords((left_margin-0.9)/fig_width,
                              ...
                              fig.transFigure)
    ax.text(0.7, 0.7, 'right atom', transform=ax.transAxes)
    ...
    set_axis(ax, 'y', 0, 150, 50,  minor=2)
    ax.set_xticklabels([])
    ax.plot(tgrid, np.abs(pulse_blue_left.amplitude), color=lightblue)
    line, = ax.plot(tgrid, np.abs(pulse_red_left.amplitude), color=red)
    line.set_dashes([8,8])
    ax.text(0.2, 0.7, 'left atom', transform=ax.transAxes)
    ...
    ax.legend((legend_blue_line, legend_red_line),
              ('$\Omega_B(t)$', '$\Omega_R(t)$'),
              loc='center', bbox_to_anchor=(0.7, 0.5))
...
def read_data(data_folder):
    pulse_blue_left  = Pulse(filename=os.path.join(data_folder, 'jz_pulse1.dat'))
    pulse_red_left   = Pulse(filename=os.path.join(data_folder, 'jz_pulse2.dat'))
    pulse_blue_right = Pulse(filename=os.path.join(data_folder, 'jz_pulse3.dat'))
    pulse_red_right  = Pulse(filename=os.path.join(data_folder, 'jz_pulse4.dat'))
